...
function Home() {
  ...
  const addTokenToWallet = async (token: typeof celoTokens[0]) => {
    ...
    try {
      ...
      console.error("Adding token failed:", err);
    } finally {
      setAddingToken(null);
    }
  };

  // if metamask open hangs, open the WalletConnect modal
  const handleOpenConnect = async (openModal: () => void) => {
    setIsRainbowLoading(true);
    try {
      // On mobile, window.ethereum.request('eth_requestAccounts') may hang
      if (typeof window !== "undefined" && (window as any).ethereum?.request) {
        ...
        throw new Error("No injected eth provider");
      }
    } catch {
      openModal();
    } finally {
  ...
  return (
    <div className="w-full h-full flex items-center justify-center p-4 overflow-hidden">
        ...
        <div className="w-full max-w-3xl relative z-10">
          ...
          <div className="flex justify-center">
            <ConnectButton.Custom>
              {({ openConnectModal }) => (
                <motion.button
                  onClick={() => handleOpenConnect(openConnectModal)}
                  disabled={isRainbowLoading}
                  className={`w-full flex flex-col items-center justify-center p-10 rounded-2xl backdrop-blur-2xl border border-white/20 transition-all duration-500 hover:border-white/40 ${
                    isRainbowLoading ? "opacity-80 cursor-not-allowed" : ""
                  }`}
                  style={{
                    background:
                      "radial-gradient(circle at center, rgba(55, 48, 107, 0.3) 0%, rgba(30, 27, 75, 0.3) 100%)",
                    boxShadow: "0 0 30px rgba(99, 102, 241, 0.3), inset 0 0 20px rgba(199, 210, 254, 0.1)"
                  }}
                  whileHover={{ scale: 1.03, boxShadow: "0 0 30px rgba(139, 92, 246, 0.4), inset 0 0 25px rgba(165, 180, 252, 0.2)" }}
                >
                  {/* Icon */}
                  <div
                    ...
                    style={{
                      ...
                      boxShadow: "inset 0 0 20px rgba(199, 210, 254, 0.3), 0 0 30px rgba(99, 102, 241, 0.4)"
                    }}
                  >
                    <motion.div
                    ...
                    />
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="w-16 h-16 fill-none stroke-current text-transparent">
                      <defs>
                  ...
                  <motion.h3
                    ...
                    animate={{ textShadow: ["0 0 10px rgba(165,180,252,0.5)", "0 0 15px rgba(199,210,254,0.7)", "0 0 10px rgba(165,180,252,0.5)"] }}
                  ...
                  {isRainbowLoading ? (
                    <div className="flex items-center mt-3">
                      <svg className="animate-spin h-6 w-6 mr-2 text-[#a5b4fc]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                        <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                        <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                      </svg>
                      <motion.span className="text-sm font-light text-[#e0e7ff]" animate={{ opacity: [0.7, 1, 0.7] }} transition={{ duration: 2, repeat: Infinity }}>
                        Opening Stargate...
                      </motion.span>
                    </div>
                  ) : (
                    <motion.p className="text-sm mt-2 font-light text-[#c7d2fe]" animate={{ opacity: [0.7, 1, 0.7], y: [0, -2, 0] }} transition={{ duration: 3, repeat: Infinity }}>
          ...
          <motion.p className="text-center mt-10 text-sm font-light max-w-md mx-auto text-[#a5b4fc]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2, duration: 1 }}>
            Secure connection through cosmic gateways to 150+ celestial wallets
          </motion.p>
        </div>
      )}

      {/* NETWORK SWITCH + TOKEN LIST VIEW */}
      {isConnected && address && needsNetworkSwitch && (
        <div className="w-full max-w-3xl relative z-10">
          <motion.h1
            className="text-3xl md:text-4xl font-bold text-center mb-10"
            initial={{ opacity: 0, y: -20 }}
          ...
          <motion.div
            className="bg-gradient-to-br from-cyan-900/20 to-teal-800/10 backdrop-blur-xl rounded-2xl border border-cyan-500/30 p-8 mb-10"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.3, duration: 0.7 }}
            style={{ boxShadow: "0 0 30px rgba(6, 182, 212, 0.3), inset 0 0 20px rgba(103, 232, 249, 0.1)" }}
          >
            <div className="flex items-center justify-center mb-6">
              <motion.div className="bg-gradient-to-r from-cyan-500 to-teal-400 p-4 rounded-full" animate={{ rotate: [0, 10, -10, 0] }} transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}>
            ...
            <motion.button
              ...
              className="w-full py-4 bg-gradient-to-r from-cyan-500 to-teal-400 rounded-xl text-cyan-900 font-bold flex items-center justify-center hover:from-cyan-400 hover:to-teal-300 transition-all"
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }}>
            <h2 className="text-xl font-bold text-center mb-8 text-cyan-50">Cosmic Assets</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              {celoTokens.map((token) => (
                <motion.div
                  key={token.symbol}
                  className="bg-cyan-900/20 backdrop-blur-md rounded-xl p-5 border border-cyan-500/30 flex items-center"
                  whileHover={{ y: -5, boxShadow: "0 5px 15px rgba(6, 182, 212, 0.2)" }}
                ...
                >
                  ...
                  <button
                    ...
                    className="bg-cyan-700/50 hover:bg-cyan-600/60 px-4 py-2 rounded-lg text-cyan-100 text-sm font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      {isConnected && address && !needsNetworkSwitch && <Spin />}
    </div>
  );
}
