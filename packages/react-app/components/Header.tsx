...
import { VortexAddress, SUPPORTED_TOKENS } from "@/app/config/addresses";

// Helper function to format CUSD from wei
function formatCUSDFromWei(wei: bigint): string {
  if (wei === 0n) return "0.00";

  const rounding = 500000000000000000n; // 0.5 * 10^18
  const total = (wei * 100n + rounding) / 1000000000000000000n;
  const integerPart = total / 100n;
  const decimalPart = total % 100n;
...
export default function Header() {
  ...
  const [onchainBalance, setOnchainBalance] = useState<string>("_");
  ...
  const fetchBalances = useCallback(async () => {
    ...
    try {
      ...
      const balanceWei = BigInt(resp.balance);
      setOffchainBalance(formatCUSDFromWei(balanceWei));
    } catch (err) {
    ...
    try {
      ...
      const balanceWei = await getTokenBalance(address, cUSDToken);
      setOnchainBalance(formatCUSDFromWei(balanceWei));
    } catch (err) {
      setOnchainBalance("_");
    } finally {
      setLoadingOnchain(false);
    }
  }, [address, getUserAddress, getTokenBalance]);
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[1000] flex">
          ...
          >
            ...
            <div 
              className="relative z-10 mt-6 mb-8 text-center p-6 bg-gradient-to-r from-indigo-800/50 to-purple-800/50 rounded-xl border border-cyan-400/30 backdrop-blur-sm overflow-hidden cursor-pointer"
            ...
            >
              ...
              <div className="mt-3 pt-3 border-t border-cyan-400/20">
                <div className="text-xs text-amber-300 mb-1">Onchain Balance</div>
                {loadingOnchain ? (
